	...
	foreach( $meta_box['fields'] as $field ){
		...
		switch( $field['type'] ){	
							...
							insert: {
								...
								click: function() {
									var models = frame.state().get('selection'),
										...
										this.el.innerHTML = '<?php _e("Saving...", "york-pro"); ?>';
			...
			?>
				...
				jQuery(function($) {
					...
					$('#bean_images_upload').on('click', function(e) {
						...
						//FIRST FRAME
						var options = {
							title: '<?php _e("Create Gallery", "york-pro"); ?>',
							state: 'gallery-edit',
							frame: 'post',
							selection: selection
						};
						//IF FRAME IS THERE
						if( frame || selection ) {
							options['title'] = '<?php _e("Edit Gallery", "york-pro"); ?>';
						}

						frame = wp.media(options).open();
						
						// CUSTOMIZE VIEWS
						frame.menu.get('view').get('cancel').el.innerHTML = '<?php _e("Cancel", "york-pro"); ?>';
						frame.menu.get('view').get('gallery-edit').el.innerHTML = '<?php _e("Edit Gallery", "york-pro"); ?>';
						...
						function overrideGalleryInsert() {
    						frame.toolbar.get('view').set({
								insert: {
									style: 'primary',
									text: '<?php _e("Save Gallery", "york-pro"); ?>',
			...
			<?php
				...
				$thumbnail_output = '';
				$button_text = ($meta) ? __('Edit Gallery', 'york-pro') : $field['std'];
				if( $meta ) {
					$field['std'] = __('Edit Gallery', 'york-pro');
					$thumbs = explode(',', $meta);
					$thumbnail_output = '';
			...
			case 'checkbox':
                ...
                <input type="checkbox" id="'. $field['id'] .'" name="bean_meta['. $field['id'] .']" value="on"'. $val .' /><span>'. __("Yes, please do", "york-pro") .'</span>';
			    echo '</td>';
			    break;
		}
